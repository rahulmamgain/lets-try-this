...
(function() {
  var EPub, Q, _, cheerio, ejs, exec, fs, fsextra, mime, path, removeDiacritics, request, uslug, uuid,
  ...
  EPub = (function() {
    function EPub(options, output) {
      ...
      if (!options.title || !options.chapters) {
...
this.options.chapters.forEach(function(chapter,cIndex) {

    chapter.subChapters = chapter.subChapters.map((subChapter,sIndex) => {
        
        var $, allowedAttributes, allowedXhtml11Tags, titleSlug;
        titleSlug = uslug(removeDiacritics(subChapter.title || "no title"));
        subChapter.filePath = path.resolve(self.uuid, `./OEBPS/${chapter.title}/` + titleSlug + ".xhtml");
        subChapter.href = `${chapter.title}/` + titleSlug + ".xhtml";
        subChapter.id = "item_" + cIndex + "_" + sIndex;
        subChapter.author = subChapter.author && _.isString(subChapter.author) ? [subChapter.author] : !subChapter.author || !_.isArray(subChapter.author) ? [] : subChapter.author;
        ...
        subChapter.data = subChapter.data.replace(/\&/g, "&amp;");
        $ = cheerio.load(subChapter.data, {
          xmlMode: true,
          lowerCaseTags: true,
    ...
    EPub.prototype.generateTempFile = function() {
         var base, generateDefer, htmlTocPath, ncxTocPath, opfPath,opf2path, self;
         ...
         this.options.chapters.forEach((chapter) => {
           fs.mkdirSync(path.resolve(this.uuid, `./OEBPS/${chapter.title}`));

           chapter.subChapters.forEach((content) => {
             var data;
         ...
         fs.writeFileSync(this.uuid + "/META-INF/container.xml", "<?xml version=\"1.0\" encoding=\"UTF-8\" ?><container version=\"1.0\" xmlns=\"urn:oasis:names:tc:opendocument:xmlns:container\"><rootfiles><rootfile full-path=\"OEBPS/content2.opf\" media-type=\"application/oebps-package+xml\"/></rootfiles></container>");
         if (self.options.version === 2) {
         ...
         opf2path = path.resolve(__dirname, "../templates/epub" + self.options.version + "/content2.opf.ejs");
         ...
         ncx2TocPath = path.resolve(__dirname, "../templates/toc2.ncx.ejs");
         ...
         Q.all([
          Q.nfcall(ejs.renderFile, opf2path, self.options), 
          Q.nfcall(ejs.renderFile, ncx2TocPath, self.options)]).spread(function(data11, data21, data3) {
           fs.writeFileSync(path.resolve(self.uuid, "./OEBPS/content2.opf"), data11);
           fs.writeFileSync(path.resolve(self.uuid, "./OEBPS/toc2.ncx"), data21);
