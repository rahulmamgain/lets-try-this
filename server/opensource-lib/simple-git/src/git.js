(function () {
  ...
  /**
    ...
    * @param {Function} [then]
    */
   Git.prototype.show = function (options,binary,then) {
      ...
      if (binary) {
        return this._runBinary(command, function (err, data) {
   ...
   Git.prototype._runBinary = function (command, then) {
      if (typeof command === "string") {
         command = command.split(" ");
      }
      this._runCache.push([command, then]);
   ...
   Git.prototype._schedule = function (binary) {
      if (!this._childProcess && this._runCache.length) {
         ...
         spawned.on('close', function (exitCode, exitSignal) {
            ...
            if (exitCode && stdErr.length) {
              ...
              if (binary) {
                then.call(this, null, Buffer.concat(stdOut));
              } else {
